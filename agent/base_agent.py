                ...
                for index in monopolies:
                    if s.agentSign() == -1:
                        eachMonopoly = sorted(monopolies[index].items(), key=lambda x: x[1],
                                              reverse=True)  # Tile with the least number of houses comes first if agent is -1
                    elif s.agentSign() == 1:
                        eachMonopoly = sorted(monopolies[index].items(), key=lambda x: x[
                            1])  # Tile with the least number of houses comes first if agent is +1
                    for prop in eachMonopoly:  # Should not cause issues with eachMonopoly as agentSign() is +1 or -1
                        v = abs(s.properties()[prop[0]])  # This gives us the value associated with the property 1 - 7
                        p = board[prop[0]]["build_cost"]
                        if (
                                money_spent + p >= money * self._buyPct):  # Try to break loop if money spent + next house cost is more than 40% (self._buyPct) of available cash
                            continueLoop = False
                            break
                ...
                for eachMonopoly in monopolies:
                    for eachProp in eachMonopoly:
                        ...
                        if abs(v) == 1:#Unimproved property
                            return (board[v])['price'] * self._dickness
                        elif 1 < abs(v) < 7:
                            return ((board[v])['price'] + ((board[v])['build_cost'] * abs(v)-1)) *  self._dickness
                        elif abs(eachProp) == 7:
                            return ((board[v])['price'] * 2 * 1.1) *  self._dickness
